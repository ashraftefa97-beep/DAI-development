...
const authBootstrap=readFileSync(new URL('../src/authBootstrap.ts',import.meta.url),'utf8');
...
test('social wave does not masquerade as a success event',()=>assert.match(director,/wave/));
...
test('real motion engine keeps head-only gestures hand-free across all avatars',()=>assert.match(motion,/head/i));
...
test('speech mood returns intensity instead of a flat label only',()=>assert.match(director,/intensity/i));
